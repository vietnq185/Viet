...
Utils.isAppError = function (str) {
  var arr = Object.values(_constants2.default.errors);
  return arr.indexOf(str) !== -1;
};

Utils.sha3Encrypt = function (str) {
  var SHA3_OPTS = { outputLength: 256 };
  return sha3(str, SHA3_OPTS).toString();
};

Utils.aesEncrypt = function (str, secret) {
  str += '';
  return aes.encrypt(str, Utils.sha3Encrypt(secret)).toString();
}; // eslint-disable-line

Utils.aesDecrypt = function (encryptedStr, secret) {
  // eslint-disable-line
  encryptedStr += '';
  var bytes = aes.decrypt(encryptedStr, Utils.sha3Encrypt(secret));
  try {
...
Utils.isNotEmptyObject = function (obj) {
  return (typeof obj === 'undefined' ? 'undefined' : _typeof(obj)) === 'object' && obj !== null && obj.constructor !== Array && Object.keys(obj).length > 0;
};
...
* @return {object}
*/
var mergeTwoObjects = function mergeTwoObjects(objSrc, objDst) {
...
/**
   ...
   * @returns {Promise}
   */
Utils.getDataPair = function (results, arrKeyFields) {
  ...
  try {
    ...
    //
    var getKey = function getKey(item, fields, separator) {
    ...
    //
    var getValue = function getValue(item, fields) {
    ...
    for (var i = 0; i < results.length; i++) {
      ...
      var item = results[i];
      //
      var key = getKey(item, arrKeyFields, strKeySeparator);
      //
      var value = Utils.isNotEmptyArray(arrResultFields) ? getValue(item, arrResultFields) : item; // eslint-disable-line
      //
